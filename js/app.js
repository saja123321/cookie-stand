...
CookieStand.prototype.customersPerHour = function () {
  for (let i = 0; i < hours.length; i++) {
    this.hourlyCustomers[i] = Math.floor((Math.random() * (this.max - this.min + 1) + this.min) * this.avg);
...
CookieStand.prototype.render = function () {
  ...
  for (let index = 0; index < hours.length; index++) {
    ...
    tdE.textContent = this.hourlyCustomers[index];
    totalOfTotal += this.hourlyCustomers[index];
    trE.appendChild(tdE);
  }
  let tdE = document.createElement('td');
